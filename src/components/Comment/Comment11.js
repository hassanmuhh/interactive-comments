...
const Comment = ({ content, createdAt, id, score, username, userImg, replies = false, replyingTo = false, currentUser, newComment }) => {
  ...
  let hasReplies = replies !== 'false' && replies.length > 0;
  ...
  return (
    <>
      {!deleteComment && (
        <div className={classes.commentWrapper}>
          ...
          <div className={!newComment ? classes.voteArea : `${classes.voteArea} ${classes.newComment}`}>
            <button className={classes.btn} onClick={() => { setScoreNum(prev => ++prev) }}><FaPlus /></button>
            <p>{scoreNum}</p>
            <button className={classes.btn} onClick={() => { setScoreNum(prev => --prev) }}><FaMinus /></button>
          </div>
          ...
          <div className={classes.commentContent}>
            <div className={classes.commentHead}>
              <div className={classes.userInfo}>
                {userImg && (
                  <img className={classes.userImg} src={userImg.png} alt="user profile" />
                )}
            ...
            <div className={turnToInput ? classes.turnedInput : classes.commentText}>
              <p ref={editCommentRef} contentEditable={turnToInput}>{replyingTo && <span className={classes.replyingTo}>@{replyingTo}</span>} {content}</p>
      ...
      {hasReplies && <div className={classes.replyArea}>
        {repliesList.map(reply => {
          return <Reply key={reply.id} id={reply.id} content={reply.content} replyingTo={reply.replyingTo} score={reply.score} userImg={reply.user.image} userName={reply.user.username} createdAt={reply.createdAt} />
        })}
      </div>
      }
      {newReply.length > 0 ? newReply.map((rep, idx) => {
        return <NewComment key={idx} info={rep} replyTo={username} />
      }) : ''
      }
